...
const getInfoForBranches = async (baseUrl, branches) => {
  const [...branchData] = await Promise.all([...branches.map(getSnapshotInfo)]);
  return branchData;
};

const getSnapshotInfo = async (branch) => {
  const { version } = await getKibanaPackage(branch);
  const [latest, latestVerified] = await Promise.all([getManifest(version, false), axios.get(`https://storage.googleapis.com/kibana-ci-es-snapshots-daily/${version}/manifest-latest-verified.json`)]);
